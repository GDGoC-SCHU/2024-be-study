...
public class ArticleController {
    ...
    public List<ArticleResponseDto> getArticlesByAuthor(@RequestParam String author) {
        List<ArticleResponseDto> response = articleService.getArticlesByAuthor(author);
        return response;
    }
    ...
    public List<ArticleResponseDto> getArticlesByAuthor(@PathVariable String author) {
    ...
    public ResponseEntity<String> deleteAllArticles() {
        String result = articleService.deleteAllArticles();
    ...
    // 총 3개의 api
    //ArticleService 클래스에는 이미 delete 메소드가 구현 추가코드 x
    //ArticleRepository 인터페이스에는 deleteById 메소드를 사용하므로 추가적인 메소드 x
    @DeleteMapping("/article-delete-path/{id}")
    public ResponseEntity<String> deleteArticleByPath(@PathVariable Long id) {
        String message = articleService.delete(id);
        return ResponseEntity.status(HttpStatus.OK).body(message);
    }

    @DeleteMapping("/article-delete-param")
    public ResponseEntity<String> deleteArticleByParam(@RequestParam Long id) {
        String message = articleService.delete(id);
        return ResponseEntity.status(HttpStatus.OK).body(message);
    }

    @DeleteMapping("/article-delete-body")
    public ResponseEntity<String> deleteArticleByBody(@RequestBody ArticleRequestDto requestDto) {
        String message = articleService.delete(requestDto.getId());
        return ResponseEntity.status(HttpStatus.OK).body(message);
    }
