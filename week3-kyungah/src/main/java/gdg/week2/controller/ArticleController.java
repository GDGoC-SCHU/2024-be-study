...
import gdg.week2.dto.article.ArticleCreateDto;
import gdg.week2.dto.article.ArticleRequestDto;
import gdg.week2.dto.article.ArticleResponseDto;
import gdg.week2.dto.article.ArticleUpdateDto;
...
public class ArticleController {
    ...
    @GetMapping("/article1")
    public ArticleResponseDto getArticle1() {
    ...
    @GetMapping("/article")
    public ResponseEntity<ArticleResponseDto> getArticleByRequestParam(@RequestParam String author) {
        ArticleResponseDto articleResponseDto = articleService.getArticleByAuthor(author);

        if (articleResponseDto == null) return ResponseEntity.status(HttpStatus.NOT_FOUND).body(null);
        return ResponseEntity.ok(articleResponseDto);
    }

    // 2. pathVariable 방식으로 author를 검색하여 article을 찾는 api (GetMapping)
    @GetMapping("/article/{author}")
    public ResponseEntity<ArticleResponseDto> getArticleByPathVariable(@PathVariable String author) {
        ArticleResponseDto articleResponseDto = articleService.getArticleByAuthor(author);
    ...
    @DeleteMapping("/article-delete-all")
    public ResponseEntity<String> deleteAll() {
        String message = articleService.deleteAll();

        if (message.equals("success")) return ResponseEntity.ok(message);
