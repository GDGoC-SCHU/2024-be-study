...
public class ArticleController {
    ...
    @GetMapping("/article-author-param")
    ...
    @GetMapping("/article-author-path/{author}")
    ...
    @DeleteMapping("/articles-delete-all")
    public ResponseEntity<String> deleteAllArticles() {
        String message = articleService.deleteAllArticles();
        return ResponseEntity.status(HttpStatus.OK).body(message);
    }

    // 4. 데이터 전송 방식 3가지로 특정 ID의 게시글을 삭제하는 API
    // (1) pathVariable 방식
